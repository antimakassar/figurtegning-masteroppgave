...
class Canvas:
    ...
    def math_to_px(self, x, y):
        # print('z*self.scale + self.origin_X is', x*self.scale + self.origin_X)
        X = np.rint(x*self.scale + self.origin_X)
        Y = np.rint(-(y*self.scale) + self.origin_Y)
        return X, Y

    def draw_point(self, point, color="black"):
        """blabla point is a vector (array)"""
        # print('point is',point)
        # print('np.rint(point[0][0]) is', np.rint(point[0][0]))
        X, Y = self.math_to_px(point[0], point[1])
        self.cv.create_oval(X-1, Y-1, X+2, Y+2, fill=color, outline=color, width=3)

    # def draw_point(self, point, color="black"):
    #     ''' Here point is a point in the plane given by a complex coordinate '''
    #     X, Y = self.math_to_px(point.real, point.imag)
    #     self.cv.create_oval(X-1, Y-1, X+2, Y+2, fill=color, outline=color, width=3)
